...
const srcDir =
  "/Users/garystringham/github-revrebel/Migration/REBEL-APP/packages/ui/src";
const tsconfigPath =
  "/Users/garystringham/github-revrebel/Migration/REBEL-APP/tsconfig.json";
...
if (exportMap["@buttons"]) {
  ...
  exportMap["@buttons"]["ButtonGroupText"] =
    "@buttons/input-group-button-group";
...
function rewriteFileImports(filePath) {
  ...
  newContent = content.replace(
    importRegex,
    (match, isType, specifiersStr, importSource) => {
      ...
      if (unmapped.length > 0) {
        console.warn(
          `[WARNING] In file ${filePath}: Unmapped specifiers in import from "${importSource}": ${unmapped.join(", ")}`,
        );
      }
      ...
      for (const [subpath, specs] of Object.entries(subpathGroups)) {
        newImports.push(
          `import ${typePrefix}{ ${specs.join(", ")} } from "${subpath}"`,
        );
      }
      ...
      if (unmapped.length > 0) {
        ...
        newImports.push(
          `import ${typePrefix}{ ${unmapped.join(", ")} } from "${guessedSubpath}"`,
        );
      }
