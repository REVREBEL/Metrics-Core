...
## 2026-07-28 - [TanStack Table Columns Instability & Expensive BQ Filter Queries]
**Learning:** In interactive tables, recreating column definitions on every local state update (e.g. typing cell edits) completely breaks TanStack Table's optimization, tearing down and rebuilding all cells and inputs on every keystroke. Using table custom `meta` keeps column references stable. On the backend, querying live BigQuery for filter options scanned entire datasets repeatedly. Caching these with a memory Map + TTL saves significant latency and query costs.
**Action:** Always keep column array references stable by passing dynamic state (like cell edits) to the table instance `meta` property, and cache read-only metadata lookups like filter options on the server.
