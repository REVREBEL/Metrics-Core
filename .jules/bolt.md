...
## 2026-07-28 - [Intl.NumberFormat Instantiation Overhead in Render Pass]
**Learning:** Instantiating `Intl.NumberFormat` on every single render in highly recursive or frequently rendered components (such as display metrics or cells in grid lists) degrades performance. Re-creating these formatters can be up to 100x more expensive than executing the format operation itself. Caching formatters using a Map-based lookup with a serialized parameters key (e.g., `locale:style:currency:fractionDigits`) resolves this completely while preventing memory leaks.
**Action:** Always use a static or map-based cache for `Intl.NumberFormat` instances in frequently rendered components, especially when formatting options (such as locale or decimal places) are predictable and highly repetitive.
