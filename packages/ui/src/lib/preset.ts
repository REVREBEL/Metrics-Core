import { buildGlobalCSSFromTheme } from "@/lib/playground/build-globals-css";
import { FONTS_DATA, THEMES_DATA } from "@/lib/playground/preset-data";
...
export function buildPresetCSS(config: PresetConfig): string | null {
  ...
  const baseColorName = config.baseColor === "gray" ? "zinc" : config.baseColor;
