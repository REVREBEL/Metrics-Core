...
import { getSourceRootsForItems, SKIP_FILE_PATTERNS, toManifestPath, toTitle } from "./lib/manifest-utils.mjs"
...
async function main() {
  ...
  const filePaths = new Set(
    items.flatMap((item) =>
      (item.files || []).map((file) => String(file.path || "").replace(/\\/g, "/")),
    ),
  )
  ...
  for (const sourceRoot of getSourceRootsForItems()) {
    const sourceFiles = (await walk(sourceRoot.absRoot))
      ...
      .filter((relPath) => !SKIP_FILE_PATTERNS.some((pattern) => pattern.test(relPath)))
