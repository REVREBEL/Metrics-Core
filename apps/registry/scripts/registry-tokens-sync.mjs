...
const TOKEN_GROUPS = [
  {
    ...
    title: "Core Theme",
    description:
      "Standard shadcn and app-level surface, content, state, chart, and sidebar tokens.",
    patterns: [
  ...
  },
  {
    ...
    description:
      "Positive, negative, total, and public metric indicator aliases.",
  ...
  },
  {
    ...
    description:
      "Room production segment colors for total, transient, group, crew, complimentary, and other.",
  ...
  },
  {
    ...
    description:
      "OTA and source channel tokens for channel mix and ranking visualizations.",
  ...
  },
  {
    ...
    title: "Brand Palette",
    description:
      "Metrics brand palette and inverse color scales exposed to Tailwind.",
    patterns: [
      /^color-(dark-blue|dark-green|green|light-green|light-blue|yellow|orange|red|purple|smoke|grey|blue|emerald|teal|lime|cyan)-?/,
    ],
  },
  {
    id: "fonts",
...
function groupTokens(tokens) {
  ...
  const groups = TOKEN_GROUPS.map((group) => {
    const items = tokens.filter((token) => {
      if (!group.patterns.some((pattern) => pattern.test(token.name)))
        return false;
...
async function main() {
  ...
  if (!themeItem) {
    throw new Error(
      "No registry:theme item found in apps/registry/registry.json",
    );
  }
  ...
  console.log(
    `[registry:tokens:sync] merged ${Object.keys(allVars).length} vars; added light=${addedLight}, dark=${addedDark}`,
  );
}
