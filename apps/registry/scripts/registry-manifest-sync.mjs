...
const TYPOGRAPHY_FILE_OVERRIDES = {
  "typography-blockquote": "packages/ui/src/primitives/typography/TypographyBlockquoteProps.tsx",
  ...
  "typography-inline-code": "packages/ui/src/primitives/typography/TyypographyLineCode.tsx",
  "typography-large": "packages/ui/src/primitives/typography/TypographyLarge.tsx",
  ...
  "typography-muted": "packages/ui/src/primitives/typography/TypographyMuted.tsx",
  "typography-p": "packages/ui/src/primitives/typography/TypographyP.tsx",
  "typography-small": "packages/ui/src/primitives/typography/TypographySmall.tsx",
  "typography-table": "packages/ui/src/primitives/typography/TypographyTable.tsx",
}
...
function normalizedStem(file) {
  return stem(file).replace(/[^a-zA-Z0-9]/g, "").toLowerCase()
}
...
function normalizeRegistryDependencies(values, localItemNames) {
  ...
  return values.map((value) => normalizeRegistryDependency(value, localItemNames))
}
...
async function main() {
  ...
  const localItemNames = new Set((registry.items || []).map((item) => item?.name).filter(Boolean))

  const allFilesAbs = (await Promise.all(getManifestSearchRoots().map((root) => walk(root)))).flat()
  ...
  for (const [itemIndex, item] of (registry.items || []).entries()) {
    if (Array.isArray(item.registryDependencies)) {
      item.registryDependencies = dedupeDependencies(
        normalizeRegistryDependencies(item.registryDependencies, localItemNames),
      )
    }
    ...
    for (const [fileIndex, file] of (item.files || []).entries()) {
      ...
      if (!resolvedPath) {
        ...
        unresolved.push({ item: item.name, itemIndex, fileIndex, path: current })
        continue
      }
    ...
    item.files = nextFiles.filter((file) => !DROPPABLE_STALE_FILES.has(String(file.path || "")))
  }

  await fs.writeFile(REGISTRY_JSON, `${JSON.stringify(registry, null, 2)}\n`)
  console.log(`[registry:manifest:sync] updated ${updatedPaths} file paths and added ${addedTargets} targets`)
