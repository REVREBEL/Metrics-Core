...
function pushProp(props, prop) {
  if (!prop.name || props.some((existing) => existing.name === prop.name))
    return;
  props.push(prop);
}
...
function parseMetadataTs(raw) {
  ...
  if (propsBlockMatch?.[1]) {
    ...
    while (match) {
      ...
      const propDescription = body.match(
        /description\s*:\s*["'`]([\s\S]*?)["'`]/,
      )?.[1];
      if (name && type) {
        props.push({
...
async function main() {
  ...
  const existingRaw = await fs
    .readFile(REGISTRY_METADATA_JSON, "utf8")
    .catch(() => "{}");
  ...
  console.log(
    `[registry:metadata:sync] wrote ${Object.keys(next).length} entries (merged ${mergedFromColocated} colocated metadata files)`,
  );
}
