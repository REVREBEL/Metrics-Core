...
function hasNamedExport(source) {
  return (
    /\bexport\s+(?:const|let|var|function|class|type|interface|enum)\b/.test(
      source,
...
async function buildIndexContent(dir, aliases) {
  ...
  for (const file of files) {
    ...
    if (hasDefaultExport(source)) {
      lines.push(
        `export { default as ${toExportName(withoutExtension(file), usedNames)} } from "${importPath}"`,
      );
    }
  ...
  for (const alias of aliases) {
    ...
    if (!alias.alias || !alias.target || !isValidExportName(alias.alias))
      continue;
...
async function main() {
  ...
  if (check) {
    if (result.changed > 0) {
      console.error(
        `[icons:barrels] Out of date. index changes=${result.changed}`,
      );
      process.exit(1);
    }
  ...
  console.log(
    `[icons:barrels] Updated ${result.changed} index files (${result.created} created)`,
  );
}
