...
function extractItemsSource(raw) {
  ...
  if (start === -1) {
    throw new Error(`Could not find REGISTRY_ITEMS in ${GENERATED_REGISTRY_TS}`)
  }

  const arrayStart = raw.indexOf("[", start)
  const exportStart = raw.indexOf("export function getRegistryItems", arrayStart)
  const arrayEnd = raw.lastIndexOf("];", exportStart)
  if (arrayStart === -1 || arrayEnd === -1) {
    throw new Error(`Could not extract REGISTRY_ITEMS array from ${GENERATED_REGISTRY_TS}`)
  }
...
async function main() {
  try {
    ...
    console.log(`[registry:bootstrap] manifest already exists at ${REGISTRY_JSON}`)
    return
  } catch {}
  ...
  await fs.writeFile(REGISTRY_JSON, `${JSON.stringify(manifest, null, 2)}\n`, "utf8")
  console.log(`[registry:bootstrap] wrote ${items.length} items to ${REGISTRY_JSON}`)
}
