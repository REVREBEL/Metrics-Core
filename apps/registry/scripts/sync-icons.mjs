...
function normalizeSvgForJsx(svg) {
  return svg
    ...
    .replace(
      /<style>([\s\S]*?)<\/style>/g,
      (_, css) => `<style>{${JSON.stringify(css.trim())}}</style>`,
    )
    .replace(/\sstyle=(["'])(.*?)\1/g, (_, _quote, style) =>
      styleAttributeToJsx(style),
    )
...
async function renameSourceIfNeeded(sourceFile, normalizedFile, check) {
  ...
  if (await exists(normalizedFile))
    return { sourceFile: normalizedFile, renamed: false };
...
function getCompatibilityAliases(directory, originalBase, componentName) {
  ...
  return [...aliases]
    .filter(
      (alias) => alias && alias !== componentName && isValidExportName(alias),
    )
    .map((alias) => ({
      directory,
      alias,
...
async function writeAliasManifest(nextAliases, check) {
  ...
  for (const entry of current.aliases ?? []) {
    ...
    merged.set(
      `${normalized.directory}:${normalized.alias}:${normalized.target}`,
      normalized,
    );
  }
  ...
  const manifest = {
    aliases: [...merged.values()].sort((a, b) =>
      `${a.directory}/${a.alias}/${a.target}`.localeCompare(
        `${b.directory}/${b.alias}/${b.target}`,
      ),
    ),
  };
  const nextContent = `${JSON.stringify(manifest, null, 2)}\n`;
  const prevContent = await fs
    .readFile(ALIAS_MANIFEST_PATH, "utf8")
    .catch(() => null);
  ...
  for (const sourceRoot of options.sourceRoots) {
    ...
    for (const originalSourceFile of svgFiles) {
      ...
      const originalBase = path.basename(
        originalSourceFile,
        path.extname(originalSourceFile),
      );
      const normalizedBase = toExportName(originalBase);
      const normalizedSourceFile = path.join(
        path.dirname(originalSourceFile),
        `${normalizedBase}.svg`,
      );
      const renameResult = options.normalizeSource
        ? await renameSourceIfNeeded(
            originalSourceFile,
      ...
      const targetRelativePath = toTargetRelativePath(
        sourceRoot,
        renameResult.sourceFile,
      );
      ...
      const directory =
        path.relative(ICONS_ROOT, path.dirname(targetPath)) || ".";
      aliases.push(
        ...getCompatibilityAliases(directory, originalBase, componentName),
      );
    }
  }
...
function runIconBarrels() {
  const result = spawnSync(
    process.execPath,
    ["./scripts/generate-icon-barrels.mjs"],
    {
      cwd: process.cwd(),
      stdio: "inherit",
    },
...
async function main() {
  ...
  if (options.check) {
    if (
      result.normalizedSources > 0 ||
      result.writtenComponents > 0 ||
      result.aliasManifestChanged
    ) {
      console.error(
